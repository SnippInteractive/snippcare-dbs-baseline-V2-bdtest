...
CREATE PROCEDURE GetPowerBIReportMenuList
(
	@ClientId		INT
)
AS
BEGIN
		...
		BEGIN
			...
			INSERT	@ReportMenuItemTemp(Menu,ParentMenu)
			SELECT  MenuName,Null
			FROM	@splitTab
			WHERE	MenuIndex =0
		...
		UPDATE      @ReportMenuItems SET MenuLevel = null WHERE ISNULL(IsLastChild,0) = 0
		
		SELECT		MenuId,Menu as Name,DisplayName,Description,ParentMenuId,'' as ReportId,IsLastChild,Menulevel,CanAddIcon = CASE MenuLevel WHEN 3 THEN 1 ELSE 0 END 
