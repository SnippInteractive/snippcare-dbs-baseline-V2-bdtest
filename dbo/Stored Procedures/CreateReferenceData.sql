CREATE PROCEDURE [dbo].[CreateReferenceData](@newClientId int, @existingClientId int)
AS
BEGIN
    ...
    -- Get all Reference type Tables with column name Display,Version,Name  from existing database 
		select  so.name as TableName into #tempReferenceData
			...
			'NotificationTemplate','PromotionCategory','PromotionMemberProfileItemType','PromotionCriteria','State')
