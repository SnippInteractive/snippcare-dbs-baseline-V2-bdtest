...
CREATE PROCEDURE [dbo].[bws_GenerateVouchersForCampaignNode]
...
BEGIN
	...
	update Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  set fieldlist = convert(nvarchar(400),fieldlist) + convert(nvarchar(40),',voucher code') 
	...
	update Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  set fieldlist = convert(nvarchar(400),fieldlist) + convert(nvarchar(40),',voucher name') 
	where jobdetailid in (
	select jobdetailid 
	from Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobHeader h
	join Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  d on h.JobId=d.JobId
	where ActionType = 'AssignVoucher'
	and fieldlist not like '%voucher name%')

	update Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  set fieldlist = convert(nvarchar(400),fieldlist) + convert(nvarchar(40),',voucher end date') 
	where jobdetailid in (
	select jobdetailid 
	from Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobHeader h
	join Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  d on h.JobId=d.JobId
	where ActionType = 'AssignVoucher'
	and fieldlist not like '%voucher end date%')
	...
	select @ClientId=clientid from Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobHeader h join Baseline_CampaigningDB.dbo.CatalystMail_CampaignJobdetails  d on h.JobId=d.JobId
	...
	SELECT @NodeId = FilterParentId FROM Baseline_CampaigningDB.dbo.CatalystMail_Action WHERE Id = @CampaignActionId
	...
	select @DeviceProfileTemplateIdString = f.ValuesJSON from Baseline_CampaigningDB.dbo.CatalystMail_Action a 
	inner join Baseline_CampaigningDB.dbo.CatalystMail_ActionsFields af on a.Id=af.ActionId
	inner join Baseline_CampaigningDB.dbo.CatalystMail_Field f on f.Id=af.FilterId 
	...
	SELECT @VoucherExpiryDateString = ValuesJSON FROM Baseline_CampaigningDB.dbo.CatalystMail_Field AS CMF 
	INNER JOIN Baseline_CampaigningDB.dbo.CatalystMail_ActionsFields AS CMAF ON CMF.Id = CMAF.FilterId 
	...
	SELECT @VoucherStartDateString = ValuesJSON FROM Baseline_CampaigningDB.dbo.CatalystMail_Field AS CMF 
	INNER JOIN Baseline_CampaigningDB.dbo.CatalystMail_ActionsFields AS CMAF ON CMF.Id = CMAF.FilterId 
	...
	SELECT @UseProfileExpiry = UseProfileExpiry from Baseline_CampaigningDB.dbo.catalystmail_action where id=@CampaignActionId
