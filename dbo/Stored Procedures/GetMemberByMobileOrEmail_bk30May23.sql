CREATE   PROCEDURE [dbo].[GetMemberByMobileOrEmail_bk30May23] -- [GetMemberByMobileOrEmail] 2, '6093357141', 0  
@Clientid INT NULL =1 ,    
@Source NVARCHAR(50),    
@UserId INT OUTPUT        
AS    
BEGIN        
 ...
 SELECT @UserTypeId = [UserTypeId] FROM UserType WHERE [Name]='LoyaltyMember' and clientid = @clientid      
  ...
  AND  U.UserTypeId= @UserTypeId    
    ...
    WHERE cd.Email = @Source  AND U.UserStatusId =@userstatusid AND  U.UserTypeId= @UserTypeId    
    if @UserId != 0 Return -- found it, then get out      
      
       
      
    --the username is the next most obvious, check this second      
    SELECT Top 1 @UserId = u.UserId FROM [dbo].[User] u where username = @Source and U.UserStatusId =@userstatusid AND  U.UserTypeId= @UserTypeId     
    if @UserId != 0 Return -- found it, then get out      
 ...
 */  
 ...
 END  
    ...
    WHERE cd.MobilePhone = @Source OR (UA.MobilePrefix+cd.MobilePhone) = @Source COLLATE Latin1_General_CI_AS      
