...
BEGIN
   ...
   -- store all item filters to #itemcode from current trx detail table
	...
	--Fetch All Reward LineItem And basket Promotions
		...
		inner join PromotionItem pt   on p.PromotionId=pt.PromotionId
		inner join #tempCode tmp  on  pt.PromotionItemTypeId=tmp.TypeId
		...
		FROM @AllEnabledRewardOffer P  
		INNER JOIN PromotionItem pit  ON P.PromotionId = pit.PromotionId
...
--END BEST OFFER
	...
	UPDATE #VirtualStampCard SET ChildPunch = 0 WHERE Quantity <= 0 AND NetValue >= 0
	--SELECT * FROM #tempCode
