	...
	---------------------END REWARD PROMO TRX------------
			...
			--inner join Rewards rw on rw.RewardId=td.PromotionItemId		
			 ...
			 BEGIN
				...
				SELECT 0 Version,PromotionId,TrxDetailId,vs.Quantity 
				FROM #itemcode ic 
				inner join VirtualStampCard vs on ic.Linenumber = vs.LineNumber 
				WHERE vs.TrxId = @TrxId AND vs.PromotionOfferType IN('Reward','Voucher') AND ISNULL(vs.Quantity,0) > 0
				...
				BEGIN 
					...
					INSERT INTO TrxDetailStampCard 
					SELECT 0 Version,PromotionId,ic.TrxDetailId, (@VoucherQuantity * r.QualifyingProductQuantity )*-1 AS Quantity
					FROM #itemcode ic 
				...
				--SELECT * FROM TrxDetailStampCard
