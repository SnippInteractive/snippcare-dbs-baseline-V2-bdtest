...
DECLARE @AccountId INT,@EposPromotionSettings NVARCHAR(100),@BasketMaxPointsValue  DECIMAL(18,2)--@trxStatusCompleted INT
...
BEGIN
	 ...
	 UPDATE TrxDetail SET Points = 0,BonusPoints = 0  Where TrxId = @TrxId
...
BEGIN
	...
	BEGIN
		SELECT @LoyaltyDevice=deviceid,@SiteId=siteid,@reference=Reference, @TrxTypeId = TrxTypeId, @TrxDate = CreateDate from trxheader  where trxid=@TrxId
	END
	ELSE
	BEGIN
		SELECT @LoyaltyDevice=deviceid,@SiteId=siteid,@reference=Reference, @TrxTypeId = TrxTypeId, @TrxDate = TrxDate from trxheader  where trxid=@TrxId
	END
	--DECLARE @SpendToPointsConversionUnit INT = 1
	--IF ISNULL(@LoyaltyDevice,'') != ''
	--BEGIN 
	--	SELECT top 1 @SpendToPointsConversionUnit = ldpt.SpendToPointsConversionUnit from Device d  inner join DeviceProfile dp  on d.id=dp.DeviceId 
	--	inner join LoyaltyDeviceProfileTemplate ldpt  on ldpt.id = dp.DeviceProfileId
	...
	--SELECT  @MemberId = userid, @AccountId = AccountId from device  where deviceid=@LoyaltyDevice
		
		SELECT @MemberId = D.Userid,@AccountId = D.AccountId ,@BasketMaxPointsValue = lp.BasketMaxPointsValue
	...
	--SELECT  TrxDetailId,Anal1,anal2,anal3,anal4,anal5,anal6,anal7,anal8,anal9,anal10,anal11,anal12,anal13,(value * @SpendToPointsConversionUnit) as Points,LineNumber,@SiteId as SiteId,anal14,anal15,anal16,ItemCode,value, Quantity into #itemcode from TrxDetail  where trxid=@TrxId

	-- store all item filters to #itemcode from current trx detail table
	--SELECT TrxTypeId from TrxType WHERE clientid=@ClientId AND Name = 'Quantity';
	...
	--select * from [site] where clientid = 8
	--SELECT * FROM [GetParentSitesBySiteId](2056)

	DROP TABLE IF EXISTS #TrxDetail
	SELECT th.TrxId, TrxDetailId,Anal1,anal2,anal3,anal4,anal5,anal6,anal7,anal8,anal9,anal10,anal11,anal12,anal13,(value * @SpendToPointsConversionUnit) as Points,(value * @SpendToPointsConversionUnit) as BasePoints,LineNumber,@SiteId as SiteId,anal14,anal15,anal16,ItemCode,value, Quantity 
	into #TrxDetail from TrxDetail td 
	inner join Trxheader th on td.TrxId = th.TrxId  
	inner join TrxType tt on th.TrxTypeId = tt.TrxTypeId
	where th.trxid=@TrxId AND tt.Clientid=@ClientId AND tt.Name IN ('PosTransaction','Receipt')

	IF NOT EXISTS(SELECT 1 FROM #TrxDetail)
	BEGIN
		DELETE VirtualPointPromotions WHERE trxid=@Trxid
	END

	--PRINT '--------------------------------------------BasketMaxPointsValue'
	IF  ISNULL(@BasketMaxPointsValue,0) > 0
	BEGIN
		 DECLARE @SumNetValue Money,@SumBasePoints float--,@UpdateTrxDetail INT = 0

		IF NOT EXISTS (SELECT 1 	FROM Promotion p 
						inner join PromotionItem Pt on p.Id = Pt.PromotionId 
						INNER JOIN PromotionOfferType po on p.PromotionOfferTypeId = po.Id 
						WHERE  pt.Code  in ('Other','Item(s)') AND EndDate > GETDATE() AND po.Name = 'PointsMultiplier' AND po.ClientId = @ClientId AND P.Enabled = 1)
		BEGIN
			UPDATE #TrxDetail SET Points = 0,BasePoints = 0 where ItemCode in ('Other','Item(s)') AND ISNULL(Points,0) <> 0
		END

		 SELECT @SumNetValue = SUM(Value),@SumBasePoints = SUM(ISNULL(BasePoints,0)) FROM #TrxDetail WHERE ISNULL(Points,0) > 0 AND ISNULL(Value,0) > 0 AND ItemCode NOT in ('Other','Item(s)')

		 IF  ISNULL(@BasketMaxPointsValue,0) > 0 AND ISNULL(@SumNetValue,0) > 0 AND ISNULL(@SumBasePoints,0) > ISNULL(@BasketMaxPointsValue,0)
		 BEGIN
			UPDATE #TrxDetail SET Points = (((ISNULL(Value,0) / ISNULL(@SumNetValue,0)) * ISNULL(@BasketMaxPointsValue,0))) , BasePoints = (((ISNULL(Value,0) / ISNULL(@SumNetValue,0)) * ISNULL(@BasketMaxPointsValue,0)))
			where ISNULL(Points,0) > 0 AND ISNULL(Value,0) > 0 AND ItemCode NOT in ('Other','Item(s)')
			--SET @UpdateTrxDetail = 1
		 END
	 END
	--PRINT '--------------------------------------------BasketMaxPointsValue'
	-----------------------------------------------------------------------------------------
	...
	SET @RoundingType  =	CASE @PointRule 
							...
							WHEN 'RoundPointsOnLineItemUp'	 THEN 'CEILING' 
							WHEN 'RoundPointsOnLineItemDown' THEN 'FLOOR' 
							ELSE '' END
	...
	BEGIN
		...
		--SET @UpdateTrxDetail = 1
		--update TD SET TD.Points = TTD.Points 
		--FROM TrxDetail TD INNER JOIN #TrxDetail TTD ON TD.TrxDetailId = TTD.TrxDetailId
	END
	--IF ISNULL(@UpdateTrxDetail,0) = 1
	--BEGIN
		update TD SET TD.Points = TTD.Points 
		FROM TrxDetail TD INNER JOIN #TrxDetail TTD ON TD.TrxDetailId = TTD.TrxDetailId
	--END
	--DECLARE @TrxPoints FLOAT
	--SELECT @TrxPoints = SUM(ISNULL(Points,0)) FROM #TrxDetail
	--IF ISNULL(@TrxPoints,0) != 0
	--BEGIN
	--	update trxheader set [AccountPointsBalance] = @TrxPoints where trxid=@TrxId
	--	update account set PointsBalance=isnull(PointsBalance,0)+(isnull(@TrxPoints,0)) where accountid=@AccountId
	--END

	DROP TABLE IF EXISTS #PromotionItemType
	CREATE TABLE #PromotionItemType (Id INT,Name NVARCHAR(50),ClientId INT)
	INSERT INTO #PromotionItemType (Id,Name,ClientId)
	SELECT Id,Name,clientid FROM PromotionItemType   WHERE  clientid=@ClientId
	...
	(SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode1' and clientid=@ClientId) as TypeId,anal1 as Code,SiteId,Quantity   from #TrxDetail where anal1 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode2' and clientid=@ClientId) as TypeId,anal2 as Code,SiteId,Quantity   from #TrxDetail where anal2 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode3' and clientid=@ClientId) as TypeId,anal3 as Code,SiteId ,Quantity  from #TrxDetail where anal3 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode4' and clientid=@ClientId) as TypeId,anal4 as Code,SiteId ,Quantity  from #TrxDetail where anal4 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode5' and clientid=@ClientId) as TypeId,anal5 as Code,SiteId ,Quantity  from #TrxDetail where anal5 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode6' and clientid=@ClientId) as TypeId,anal6 as Code,SiteId ,Quantity  from #TrxDetail where anal6 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode7' and clientid=@ClientId) as TypeId,anal7 as Code,SiteId ,Quantity from #TrxDetail where anal7 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode8' and clientid=@ClientId) as TypeId,anal8 as Code,SiteId ,Quantity  from #TrxDetail where anal8 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode9' and clientid=@ClientId) as TypeId,anal9 as Code,SiteId ,Quantity  from #TrxDetail where anal9 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode10' and clientid=@ClientId) as TypeId,anal10 as Code,SiteId ,Quantity  from #TrxDetail where anal10 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode11' and clientid=@ClientId) as TypeId,anal11 as Code,SiteId ,Quantity  from #TrxDetail where anal11 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode12' and clientid=@ClientId) as TypeId,anal12 as Code,SiteId ,Quantity  from #TrxDetail where anal12 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode13' and clientid=@ClientId) as TypeId,anal13 as Code,SiteId ,Quantity  from #TrxDetail where anal13 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode14' and clientid=@ClientId) as TypeId,anal14 as Code,SiteId ,Quantity  from #TrxDetail where anal14 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode15' and clientid=@ClientId) as TypeId,anal15 as Code,SiteId ,Quantity from #TrxDetail where anal15 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='AnalysisCode16' and clientid=@ClientId) as TypeId,anal16 as Code,SiteId ,Quantity  from #TrxDetail where anal16 is not null
	union
	SELECT TrxDetailId,Points,value,LineNumber,( SELECT id from #PromotionItemType where name='ItemCode' and clientid=@ClientId) as TypeId,itemcode as Code,SiteId ,Quantity from #TrxDetail where itemcode is not null
...
--END TOP 666
	...
	inner join #TrxSite ts on vs.SiteId = ts.SiteId
	...
	vsp.Name in('PointsMultiplier','PointsFixed') 
	--and tmp.SiteId in( SELECT SiteId from GetChildSitesBySiteId(vs.siteid))
	...
	BEGIN
		...
		BEGIN  
			...
			BEGIN
				...
				SELECT @MaxUsagePerMemberLimit = isnull(MaxUsagePerMember,0), @PromotionUsageLimit= isnull(PromotionUsageLimit,0) from Promotion  where Id =@PromotionId
	...
	BEGIN
		...
		BEGIN
			...
			join LoyaltyDeviceProfileTemplate ldpt  on dpt.id = ldpt.id
	...
	update account set @PointsBalance = PointsBalance, PointsBalance=isnull(PointsBalance,0)+isnull(@BaseandBonusPointsTotal,0) where accountid= @AccountId--(SELECT accountid from device where deviceid=@DeviceId)

	update trxheader set [AccountPointsBalance]=isnull(@PointsBalance,0)+(isnull(@BaseandBonusPointsTotal,0)) ,TotalPoints = isnull(@BaseandBonusPointsTotal,0),MemberId = @MemberId  where trxid=@TrxId--,TrxStatusTypeId = @trxStatusCompleted
	...
	IF exists(SELECT 1 from memberlink m  inner join memberlinktype mt  on m.linktype=mt.MemberLinkTypeId where name='Community' and MemberId2=@MemberId and CommunityId is not null)
	BEGIN	
		exec EPOS_ShadowPointTransfer @MemberId,@LoyaltyDevice,@TrxId	
	END
	-----------------------------------------------------------------------------------------
	print 'Start voucher usage checker at ' + convert(varchar(25), getdate(), 120)
	-----------------------------------------------------------------------------------------
	DECLARE @DeviceStatusBlocked INT
	SET @DeviceStatusBlocked=(SELECT devicestatusid from Devicestatus  where name='Inactive' and clientid= @ClientId)--(SELECT clientid from client  where name='baseline'))
	...
	BEGIN  
		...
		IF(@Maxusage>0)
		BEGIN
			IF not exists(SELECT 1 from [ClassicalVoucherRedemptionCount]  where trxid=@Trxid and MemberId=@MemberId and VoucherId=@VoucherId)
			BEGIN
				INSERT INTO [dbo].[ClassicalVoucherRedemptionCount]([MemberId],[VoucherId],[LastRedemptionDate],[TrxId])
				SELECT @MemberId,@VoucherId,GETDATE(),@Trxid
			END
		END
		else
		BEGIN
		...
		(SELECT distinct p.id,p.MisCode from TrxDetailPromotion tp  join Promotion p  on tp.promotionid=p.id 
		...
		union 
		SELECT  p.id,p.MisCode from TrxUsedPromotions tp  join Promotion p  on tp.PromotionName=p.Name 
		...
		,[LastRedemptionDate]           
		,[TrxId],[ItemCode])
		SELECT @MemberId,p.Id,GETDATE(),@Trxid,'SKU-NA' from #usedpromotion p  where not exists(SELECT 1 from [PromotionRedemptionCount]  where promotionid=p.Id and trxid=@Trxid)
  ...
  --          ), 1, 1, '')
	...
	BEGIN
	EXEC dbo.ApplyPointsForTransactionPromo @ClientId,@MemberId,@LoyaltyDevice,'Transaction','',@SiteId,@TrxId,0
	END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT TOP 1 @SegmentId = SegmentId FROM [SegmentAdmin] sa  INNER JOIN [Site] s  on sa.SiteId = s.siteid 
