...
-- =============================================
CREATE PROCEDURE [dbo].[EPOS_PointPromotionsAPI] 
	-- Add the parameters for the stored procedure here
	(@TrxId INT, @TotalBonusPoints Decimal(18,2) = 0 output)
AS
BEGIN
...
select @LoyaltyDevice=deviceid,@SiteId=siteid from trxheader where trxid=@TrxId
 ...
 select  TrxDetailId,Anal1,anal2,anal3,anal4,anal5,anal6,anal7,anal8,anal9,anal10,anal11,anal12,anal13,Points,LineNumber,@SiteId as SiteId,anal14,anal15,anal16,ItemCode
 into #itemcode
 from TrxDetail where trxid=@TrxId

 --itemcode to #table
 select * into #tempCode from
 (select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode1' and clientid=@ClientId) as TypeId,anal1 as Code,SiteId from #itemcode where anal1 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode2' and clientid=@ClientId) as TypeId,anal2 as Code,SiteId from #itemcode where anal2 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode3' and clientid=@ClientId) as TypeId,anal3 as Code,SiteId from #itemcode where anal3 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode4' and clientid=@ClientId) as TypeId,anal4 as Code,SiteId from #itemcode where anal4 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode5' and clientid=@ClientId) as TypeId,anal5 as Code,SiteId from #itemcode where anal5 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode6' and clientid=@ClientId) as TypeId,anal6 as Code,SiteId from #itemcode where anal6 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode7' and clientid=@ClientId) as TypeId,anal7 as Code,SiteId from #itemcode where anal7 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode8' and clientid=@ClientId) as TypeId,anal8 as Code,SiteId from #itemcode where anal8 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode9' and clientid=@ClientId) as TypeId,anal9 as Code,SiteId from #itemcode where anal9 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode10' and clientid=@ClientId) as TypeId,anal10 as Code,SiteId from #itemcode where anal10 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode11' and clientid=@ClientId) as TypeId,anal11 as Code,SiteId from #itemcode where anal11 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode12' and clientid=@ClientId) as TypeId,anal12 as Code,SiteId from #itemcode where anal12 is not null
 union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode13' and clientid=@ClientId) as TypeId,anal13 as Code,SiteId from #itemcode where anal13 is not null
  union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode14' and clientid=@ClientId) as TypeId,anal14 as Code,SiteId from #itemcode where anal14 is not null
  union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode15' and clientid=@ClientId) as TypeId,anal15 as Code,SiteId from #itemcode where anal15 is not null
   union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='AnalysisCode16' and clientid=@ClientId) as TypeId,anal16 as Code,SiteId from #itemcode where anal16 is not null
   union
 select TrxDetailId,Points,LineNumber,( select id from PromotionItemType where name='ItemCode' and clientid=@ClientId) as TypeId,itemcode as Code,SiteId from #itemcode where itemcode is not null

 ) t

 -- Promotions
 select distinct convert(varchar(50), p.Id )as PromotionId,p.Name,tmp.LineNumber, tmp.TrxDetailID,tmp.Points,case(pop.Name) when 'PointsMultiplier' then tmp.Points*(p.PromotionOfferValue-1)
                                                                                when 'Points' then p.PromotionOfferValue end as BonusPoints,pop.Name as OfferType,p.PromotionOfferValue, 0 as PriorityLevel,0 as Processed,'Promotion' as Offer
...
BEGIN  
	 ...
	 begin
	 update TrxDetail  set Points=TrxDetail.Points+t.BonusPoints,BonusPoints=TrxDetail.BonusPoints+t.BonusPoints from #Offer t
	  where TrxDetail.TrxDetailID=t.TrxDetailID and t.PromotionId=@PromotionId and TrxDetail.TrxDetailID=@TRXDETAILID
     end 
...
select 1,PromotionId,TrxDetailId,BonusPoints from  #Offer where processed=2 and Offer='Promotion'
	...
	update TrxDetail set BonusPoints=#Offer.BonusPoints from  #Offer where processed=2 and Offer='Promotion' and #Offer.TrxDetailId=TrxDetail.TrxDetailID
end
...
begin
	select td.trxdetailid,v.promotionvalue,v.PromotionId,v.linenumber into #pointpromotions from trxdetail td 
	join VirtualPointPromotions v on td.LineNumber=v.linenumber 
	where v.trxid=@TrxId and td.trxid=@Trxid and v.PromotionId>0
		
	update TrxDetail set BonusPoints=isnull(BonusPoints,0)+isnull(v.promotionvalue,0),Points=isnull(Points,0)+isnull(v.promotionvalue,0) from #pointpromotions v where trxdetail.TrxDetailID=v.TrxDetailID and trxdetail.trxid=@TrxId
